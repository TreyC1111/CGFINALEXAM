...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_lightsOff", lightsOff);
		...
		shader->SetUniform("u_noTexture", noTextures);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("New Effect controls"))
			{
				if (ImGui::Button("Lights Off"))
				{
					shader->SetUniform("u_lightsOff", lightsOff = 1);
				...
				{
					shader->SetUniform("u_lightsOff", lightsOff = 0);
				...
				if (ImGui::Button("AmbientSpecular"))
				{
					shader->SetUniform("u_lightsOff", lightsOff = 0);
					shader->SetUniform("u_ambient", ambient = 0);
					shader->SetUniform("u_specular", specular = 0);
				...
				if (ImGui::Button("Textures On and Off"))
				{
					if (checkTextures == true)
					{
						shader->SetUniform("u_lightsOff", lightsOff = 0);
						shader->SetUniform("u_ambient", ambient = 0);
						shader->SetUniform("u_specular", specular = 0);
				...
				if (ImGui::Button("Depth Field Blur"))
				{
					//Empty
				}
			}
		...
		Texture2D::sptr beam = Texture2D::LoadFromFile("images/waterBeamTex.png");
		Texture2D::sptr fire = Texture2D::LoadFromFile("images/fireTexture.jpg");
		...
		ShaderMaterial::sptr bottleMat = ShaderMaterial::Create();
		bottleMat->Shader = shader;
		bottleMat->Set("s_Diffuse", bottle);
		bottleMat->Set("s_Specular", stoneSpec);
		bottleMat->Set("u_Shininess", 2.0f);
		bottleMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr beamMat = ShaderMaterial::Create();
		beamMat->Shader = shader;
		beamMat->Set("s_Diffuse", beam);
		beamMat->Set("s_Specular", noSpec);
		beamMat->Set("u_Shininess", 2.0f);
		beamMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr fireMat = ShaderMaterial::Create();
		fireMat->Shader = shader;
		fireMat->Set("s_Diffuse", fire);
		fireMat->Set("s_Specular", noSpec);
		fireMat->Set("u_Shininess", 8.0f);
		fireMat->Set("u_TextureMix", 0.0f);
		...
		GameObject obj2 = scene->CreateEntity("waterBottle");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/waterBottle.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(bottleMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, 90.0f);
			obj2.get<Transform>().SetLocalScale(4.0f, 4.0f, 4.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("waterBeam");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/waterBeam.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(beamMat);
			obj3.get<Transform>().SetLocalPosition(0.55f, 0.0f, 2.2f);
			obj3.get<Transform>().SetLocalRotation(125.0f, 0.0f, 90.0f);
		...
		//EnvironmentGenerator::AddObjectToGeneration("models/simplePine.obj", fireMat, 150,
			//spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		//EnvironmentGenerator::AddObjectToGeneration("models/simpleTree.obj", fireMat, 150,
			//spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/simpleRock.obj", fireMat, 40,
